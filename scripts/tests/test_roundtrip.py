...
class RoundtripDiffTests(unittest.TestCase):
    ...
    def test_paragraph_with_both_text_and_image_emits_both_events(self):

        # 段落同时含正文文本与图片时，应按顺序同时提取文本与图片事件

        from validate_docx import DocxPackage

        p_both = (

            f'<w:p xmlns:w="{W_NS}"><w:pPr><w:pStyle w:val="Normal"/></w:pPr>'

            f'<w:r><w:t>流程文字说明</w:t></w:r>'

            f'<w:r><w:drawing><wp:inline xmlns:wp="http://schemas.openxmlformats.org/drawingml/2006/wordprocessingDrawing">'

            f'<a:graphic xmlns:a="{A_NS}"><a:graphicData uri="http://schemas.openxmlformats.org/drawingml/2006/picture">'

            f'<pic:pic xmlns:pic="http://schemas.openxmlformats.org/drawingml/2006/picture"><pic:blipFill>'

            f'<a:blip xmlns:r="{R_NS}" r:embed="rImg"/>'

            f'</pic:blipFill></pic:pic></a:graphicData></a:graphic></wp:inline></w:drawing></w:r></w:p>'
        ...
        with tempfile.TemporaryDirectory(prefix="roundtrip-") as work:
            ...
            build_package(source, [_paragraph("1", "第一章"), p_both], image_bytes=PNG1)

            pkg = DocxPackage(source)

            events = pkg.body_events()

            self.assertEqual(len(events), 3)

            self.assertEqual(events[0].kind, "H")

            self.assertEqual(events[1].kind, "P")

            self.assertEqual(events[1].value, "流程文字说明")

            self.assertEqual(events[2].kind, "I")



    def test_empty_paragraph_with_numpr_not_emitted_as_event(self):

        # 仅含 numPr 但无实质文本的空白段落，不作为业务元素事件发射

        from validate_docx import DocxPackage

        empty_num = (

            f'<w:p xmlns:w="{W_NS}"><w:pPr><w:pStyle w:val="Normal"/>'

            f'<w:numPr><w:ilvl w:val="0"/><w:numId w:val="42"/></w:numPr>'

            f'<w:vanish/></w:pPr></w:p>'
        ...
        with tempfile.TemporaryDirectory(prefix="roundtrip-") as work:
            ...
            build_package(source, [_paragraph("1", "第一章"), empty_num, _paragraph("Normal", "正文")])

            pkg = DocxPackage(source)

            events = pkg.body_events()

            # 仅第一章(H)和正文(P)，空白列表项被过滤

            self.assertEqual([e.kind for e in events], ["H", "P"])



    def test_paragraph_with_hyperlink_and_image_no_text_emits_only_image_event(self):

        # 超链接仅包裹图片且无文本时，不发射空文本 X 事件，仅发射 I 图片事件
        ...
        p_link_img = (
            ...
            f'<w:hyperlink xmlns:w="{W_NS}" w:anchor="bookmark1">'
            ...
            f'</pic:blipFill></pic:pic></a:graphicData></a:graphic></wp:inline></w:drawing></w:r>'
        ...
        with tempfile.TemporaryDirectory(prefix="roundtrip-") as work:
            ...
            build_package(source, [_paragraph("1", "第一章"), p_link_img], image_bytes=PNG1)

            pkg = DocxPackage(source)

            events = pkg.body_events()

            self.assertEqual([e.kind for e in events], ["H", "I"])
