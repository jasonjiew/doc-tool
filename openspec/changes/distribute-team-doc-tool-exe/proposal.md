...
现有工具已经能够从章节 Markdown 重建并严格校验 Word，但首次拆分仍依赖固定文件名和迁移脚本，而且产品边界被误限定为需求/详细设计。需要将它产品化为通用的大型 Word 维护工具，让成员只通过 Release 安装包完成“导入 Word、自动拆分、迭代维护、一键合并”，同时由私有 Git 保持源码、版本和发布过程可追溯。

## What Changes

- 增加多项目工作区：每个项目独立保存原始 Word、模板、Markdown、图片、复杂表格、配置、日志和输出，不把用户数据写入安装目录。
- 增加安全的首次导入流程：允许选择任意文件名、具有规范 Heading 结构的 DOCX，默认使用通用大文档模式，需求/详细设计只作为可选预设；在临时目录完成提取、拆分、校验和原子发布。
...
- `word-project-import`: 从通用或预设类型 DOCX 安全创建独立文档项目，提取模板、章节 Markdown、图片和复杂表格，并在发布项目前完成结构校验。
