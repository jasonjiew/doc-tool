...
规定右侧任务/结果 Dock 的 Codex 式步骤清单：任务运行时按管线阶段序列派生步骤（图标+文字、当前步骤高亮、总体进度、已用时间），状态覆盖待处理/进行中/成功/跳过/失败/取消；心跳任务回退为单一步骤，不伪造百分比；空闲时显示最近结果卡片或引导入口。步骤数据由既有 `TaskEvent` 的 stage/status 事件与管线阶段标签表（单一数据源）派生，不新增并行进度协议。
