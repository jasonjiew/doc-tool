...
规定桌面应用在 PySide6（Qt Widgets）上的应用外壳——`QApplication`/`QMainWindow`、菜单栏、顶部项目条、状态栏、视图四态切换、窗口生命周期与几何持久化、统一 QSS 主题与 Qt 原生高 DPI。业务层（`domain`/`application`/`adapters`）与 `TaskRunner` 线程模型保持现有依赖关系不变，不引入 Qt 依赖。
